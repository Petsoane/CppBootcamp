...
void ClapTrap::takeDamage(unsigned int amount)
{
	...
	unsigned int tmpDamageReduction = (armorDamageReduction == 0? 1: armorDamageReduction);
	unsigned int totalDamage =  (energyPoints - (amount - tmpDamageReduction));

	if ( totalDamage > 0){
		energyPoints -= (amount / tmpDamageReduction);
	}
	else {
		energyPoints = 0;
	}
